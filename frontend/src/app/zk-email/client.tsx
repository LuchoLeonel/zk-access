        ...
        {emails.map((email: any, idx: number) => (
          <tr key={email.id} className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-blue-50`}>
            ...
            <td className="p-4 text-right">
              <button
                disabled={!email.valid || status === "generating"}
                className={`px-4 py-2 text-sm rounded font-semibold ${
                  email.valid && status !== "generating"
...
export default function InboxPage() {
  ...
  const { proof, setProof } = useProofStore();
  ...
  const handleEmailProof = async (email: any) => {
    try {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <p className="mb-6 text-gray-700 text-base leading-relaxed">
        Select the email you received confirming your hiring. It should contain a subject like <strong>"Welcome aboard!!"</strong>. This message will be used to generate your <span className="font-semibold text-blue-700">Verifiable Credential</span>.
      </p>
